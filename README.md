...
## ✨ 主な新機能 (v2.0.0)
- **ControlNet-LLLite 連携**: UIから参照画像をアップロードし、自動で解像度を計算・適用。専用モデルへの切り替えも自動化。
- **AI Chat (Experimental)**: 生成した画像をローカルLLM (LM Studio) に送信し、プロンプトの改善などの相談・音声読み上げが可能。
- **向上LoRAのワンクリック適用**: Turbo LoRA, Highres Boost, Detail Boost をチェックボックス1つで簡単に適用可能。
- **ポート番号の動的変更**: SystemタブからGradioの起動ポート番号を変更でき、他のツールとの競合を防止。
- **History (履歴) 機能の強化**: 
  - オリジナル画像の高画質プレビュー機能を追加。
  - 履歴から「AI Chat」や「LLLite」へ画像をワンクリックで転送。
  - 適用された向上LoRAのON/OFFステータスも履歴に保存・表示。
- **使いやすいUIレイアウト**: アコーディオンの自動伸縮や、追従するボタンなどUXを大幅改善。

## 🚀 起動方法
`start_anima_webui.bat` を実行してください。
初回起動時に自動的にPythonの仮想環境(`venv`)が構築され、必要なライブラリがインストールされます。

## ⚙️ System設定について
UIの「System」タブから以下の設定が可能です。
- ComfyUIのURL (デフォルト: `http://127.0.0.1:8188`)
- Outputフォルダ・バックアップフォルダのパス指定
- タグのプリセット編集
- 解像度やCFG/Stepsのプリセット編集
- DeepL APIキーの設定（プロンプト翻訳用）
