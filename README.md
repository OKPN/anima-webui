🎨 Anima WebUI <small>v1.4.0</small>

Anima WebUI は、ComfyUI をバックエンドとして利用し、直感的な操作で高品質な画像生成を行うための Gradio ベースの Web UI です。 v1.4.0 では、**「WebPサムネイルシステム」**を搭載し、ギャラリー表示のパフォーマンスを劇的に向上させました。数千枚の履歴があってもメモリを圧迫せず、軽快に動作します。
🎯 想定するユーザ層
