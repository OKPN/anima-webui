# 🎨 Anima WebUI <small>v1.2.1</small>

**Anima WebUI** は、ComfyUI をバックエンドとして利用し、直感的な操作で高品質な画像生成を行うための Gradio ベースの Web UI です。
最新の v1.2.1 では、1950年から現在までの画風を自在にコントロールできる**年代混合システム**や、サーバー管理の利便性を追求した**クイックコントロール機能**を搭載しています。
...
* **Python 3.12.x**: 公式サイトからダウンロードし、インストール時に必ず **[Add Python to PATH]** にチェックを入れてください。
...
* **v1.2.1 改善**: 起動時の「cannot find the path」等のエラーメッセージを完全に除去し、クリーンな起動画面になりました。
...
| タブ名 | 機能概要 |
| --- | --- |
| **Generate** | **年代スロット & リール**: 1950年〜2026年まで、最大3つの年代を混合指定可能。 <br>

<br> **Quick Server Control**: 生成画面から直接 ComfyUI の起動確認と再起動が可能。 <br>

<br> **DeepL 翻訳**: 日本語での直感的なプロンプト入力をサポート。 |
| **History** | **完全復元履歴**: 生成時の全設定（混合年代、シード、解像度等）を完璧に復元。 <br>

<br> **URL自動修復**: スマホ連携時の画像パス問題を解決するクリーンアップ機能を搭載。 |
| **System** | **IP自動検出**: サーバーのローカルIPを自動検出し、ワンクリックでセット可能。 <br>

<br> **設定管理**: タグ、ネガティブプロンプト、解像度、外部リンク名とURLを UI 上で直接編集可能。 |
...
* **二重起動の防止**: ComfyUI が既に起動している場合、起動ボタンを押しても重複して立ち上がらない安全ロジックを搭載しています。
* **Errno 22 の回避**: 保存時の URL に含まれる空白を自動除去し、Windows でのクラッシュを防ぎます。
...
* ただし、出力データの商用利用の可否は、使用した **AIモデル（Checkpoint等）のライセンス** に直接依存します。必ずモデルごとの規約を確認してください。

---

### 📈 Version History (変遷)

* **v1.2.1 (最新)**: 外部リンク編集機能、ネガティブプロンプト位置整理、二重起動防止、ランチャー美化。
* **v1.2.0**: 年代混合スロット（1950-2026）、IP自動検出機能、履歴復元パラメータの大幅強化。
* **v1.1.2**: 履歴保存の安定性向上、バグ修正。
